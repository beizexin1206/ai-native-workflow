...
**过程产物只有在价值被接管后才是可清理的脚手架。** 是否可删不看文件后缀、命名或是不是 agent 本轮生成，
而要看它承载的知识、能力或证据是否已被更稳定的事实源完整接管。清理前先问：删除后是否仍能同等程度地理解、
复现和维护？不能就先迁移、重构或纳入正式资产；仓库整洁与可复现性冲突时，优先保证可复现。

不要生成 `SESSION_*.md`、`HANDOFF.md`、`*_PLAN.md`、`*_PROGRESS.md` 这类一次性笔记。
其中真正有价值的内容沉淀进 `docs/intent/` 或 `AGENTS.md`，完成接管后再删除脚手架。
