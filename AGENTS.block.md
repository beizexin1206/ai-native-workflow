...
`docs/templates/` 下如有项目自带的子目录，表示该项目选择保留额外的评审制品。Agent 在 `/pd`、`/p`、
`/t` 的对应阶段自动读取其中相关模板并生成派生评审视图；没有子目录就不生成。派生视图不替代 Meegle、
SPEC、代码、测试、CI、缺陷系统或 `docs/intent/`，也不增加公共命令和审批门禁。
