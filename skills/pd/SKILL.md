...
如果 `docs/templates/` 下存在项目自带的子目录，读取其中与产品评审相关的模板和使用说明：

- 有 B/C 端 PRD 模板时，按需求所属产品形态只选一种，从本轮产品需求与产品设计生成评审视图；
  不先写 C 端再转 B 端，也不让派生 PRD 成为第二套正式需求。
- 需求涉及页面或交互且有原型模板时，按项目既有原型仓库、技术栈和输出约定生成；纯后端、规则或数据需求不生成原型。

评审意见必须先写回本轮产品定义和工作项草稿，再进入确认。没有项目模板子目录时跳过本步。
...
- **项目评审视图只做兼容展示。** 内容来自本轮产品定义，不能绕过确认，也不能反向独立维护需求。
