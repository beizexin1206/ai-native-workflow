...
如果 `docs/templates/` 下存在项目自带的子目录，按其中模板和使用说明自动维护对应制品：

- 测试执行前，从 SPEC 验收标准与本轮覆盖计划生成或更新测试用例集。
- 测试执行后，从测试代码、CI、原始日志和缺陷系统生成质量报告；未运行写 `NOT RUN`，环境阻塞写 `BLOCKED`。
- 模板要求完整版本或现状文档时，只从本轮已经验证的实现生成或更新，并标明来源 commit。

这些制品是派生评审视图，不能反向维护测试结果、缺陷状态或需求，也不能把静态分析写成运行通过。
没有项目模板子目录时跳过，本节不改变下面的测试完成标准。
